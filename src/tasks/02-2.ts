...
for (const line of lines) {
  ...
  // TODO: find all numbers that is the same number repeated at least twice
  // console.log(`range: ${line}`);
  for (let i = start; i < end + 1; i++) {
    const txtValue = i.toString();
    const hasRepeat = txtValue.match(/^(?<first>[1-9]\d*)(?<second>\1+)$/m);
    if (hasRepeat) {
      const { first, second } = hasRepeat.groups ?? {};
      // console.log(`first: ${first}, second: ${second} (${txtValue})`);
      sum += i;
    }
    continue;
  }
